...
class Panel:
    ...
    def __update_panel(self) -> None:
        ...
        elif self._default_width == "inner_text" and self._width is None and header_text is not None:
            if len(header_text) + header_padding[0] + header_padding[1] > len(max(inner_text, key=len)) + inner_text_padding[0] + inner_text_padding[1]: self._width = len(header_text) + header_padding[0] + header_padding[1] + 2
            else: self._width = len(max(inner_text, key=len)) + 2 + inner_text_padding[0] + inner_text_padding[1]
        ...
        if self._header_text is None:
            ...
            if len(header_text) + 2 + header_padding[0] + header_padding[1] > terminal_width: header_text = header_text[0:(terminal_width - 2 - header_padding[0] - header_padding[1] - 3)] + "..."
            ...
            if justify_header == "start": header_position = (1+header_padding[0]) # since justify is start, we're only considering the left padding
                ...
                if width%2 != 0:
                    if len(header_text)%2 != 0: header_position = ((width-1)/2) - (len(header_text)-1)/2
                    else: header_position = (width-1)/2 - len(header_text)/2
                else:
                    if len(header_text)%2 == 0: header_position = (width/2 - 1) - len(header_text)/2
                    else: header_position = width/2 - (len(header_text)-1)/2
...
panel3.set_header(
    text = "This is a really lonnggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggggg!",
    padding = (0, 0, 0, 0),
    justify = "start"
)
